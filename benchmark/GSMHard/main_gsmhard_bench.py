...
logger = get_logger("main_gsmhard_bench")


def run_gsmhard_benchmark_in_sandbox(session, system_name):
    logger.info(f"Running GSM-Hard benchmark for system: {system_name}")
    ...
    session.copy_to_runtime(
        f"{base_path}/run_gsmhard_bench.py",
        f"/sandbox/workspace/{base_path}/run_gsmhard_bench.py",
    )
...
def main():
    ...
    try:
        ...
        if setup_sandbox_environment(session, args.reinstall):
            run_gsmhard_benchmark_in_sandbox(session, args.system)
