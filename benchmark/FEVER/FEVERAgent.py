...
def agent_node(state):
    ...
    if not messages:
        messages = [HumanMessage(content=f"Claim: {state['claim']}")]
    ...
    if iteration > 3:
        messages.append(HumanMessage(content="You have reached the maximum number of turns. You must decide on a category now."))
