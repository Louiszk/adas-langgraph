...
def run_benchmark_parallel(
    ...
    max_workers: int,
    custom_results_init: Callable[[Dict], None] = None,
    custom_results_update: Callable[[Dict, Dict], None] = None,
    custom_results_finalize: Callable[[Dict], None] = None,
    custom_print_summary: Callable[[Dict], None] = None
):
    """
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_problem), 1):
            ...
            try:
                ...
                if result_info["is_correct"]:
                    ...
                    print(f"✗ Problem {idx+1}: Incorrect. Expected: {result_info['expected']}, Got: {result_info['predicted']}")
                    results["incorrect"] += 1
    ...
    if total_attempted > 0:
        ...
        results["aggregate_metrics"]["avg_duration_per_problem"] = results["aggregate_metrics"]["total_duration_seconds"] / total_attempted
        results["aggregate_metrics"]["avg_tokens_per_problem"] = results["aggregate_metrics"]["total_tokens"] / total_attempted
