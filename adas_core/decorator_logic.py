        ...
        if line.startswith("@@"):
            ...
            if call_match:
                ...
                if open_paren_pos != -1:
                    ...
                    safe_end_idx = end_line_idx if end_line_idx is not None else i
                    if decorator_name in code_related_tools:
                        code_start_line = safe_end_idx + 1
...
def execute_decorator_tool_calls(
    ...
    code_blocks = [
        found_block["content"] for found_block in find_code_blocks(response_content) if found_block["content"]
    ]
    ...
    def add_skipped_calls_message(current_index, message):
        ...
        if remaining_calls > 0:
            tool_messages.append(
                f"{message} {remaining_calls} subsequent decorator call(s) in this response were skipped. You can make new decorator calls in your next response."
            )
    ...
    for idx, tool_call in enumerate(all_decorator_calls):
        ...
        if tool_name in available_tools:
            try:
                ...
                result = (
                    tool.func(*pos_args, **kw_args)
                    if hasattr(tool, "func") and callable(tool.func)
                    else tool.invoke(kw_args)
                )
...
def build_decorator_signatures(tools: Iterable[Callable[..., Any]], code_related_tools: Dict[str, str]) -> str:
    ...
    for func in tools:
        ...
        for param in sig.parameters.values():
            if param.name == code_param_to_exclude or param.name in framework_params_to_exclude:
                continue
