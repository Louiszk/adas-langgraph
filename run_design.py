...
def run_meta_system_in_sandbox(session: StreamingSandboxSession, problem_statement, target_name, optimize_system=None):
...
def main():
    ...
    parser.add_argument("--meta-system", default="compact_system", help="The name of the meta-system to use from the meta_systems folder.")
    parser.add_argument("--optimize-system", default=None, help="Specify target system name to optimize or change")
    parser.add_argument("--container", choices=["auto", "docker", "podman"], default="auto", help="Container runtime to use (auto will try Docker first, then Podman)")
    ...
    if not os.path.exists("materialized_meta_system/MetaSystem.py"):
        print(f"ERROR: The expected MetaSystem.py file was not found.")
        return
    print(f"Meta-system successfully built.")
