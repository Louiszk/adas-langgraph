...
def validation_function(state: Dict[str, Any]) -> Dict[str, Any]:
    initial_task = HumanMessage(state.get("initial_task"))
    ...
    llm = LargeLanguageModel(
        ...
        wrapper=validation_wrapper, model_name=validation_model,
        name="Validation", is_meta=True
    )
    ...
    if steps == 0:
        ...
        formatted_hardening_prompt = hardening_prompt.format(previous_test_cases_str=previous_test_cases_str, level=level)
        prompt_messages = [SystemMessage(content=validation_prompt), initial_task, HumanMessage(content=formatted_hardening_prompt)]
...
def initial_test_runner_function(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    test_result_str = test_system_tool.invoke({"state": state})
    ...
    verbose_test_results_content = "--- Initial Test Results ---\n" + test_result_str + \
...
def meta_agent_function(state: Dict[str, Any]) -> Dict[str, Any]:
    llm = LargeLanguageModel(wrapper=meta_agent_wrapper, model_name=meta_agent_model, reasoning_effort=meta_agent_reasoning_effort, name="MetaAgent", is_meta=True)
...
def tool_execution(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    human_message, tool_results = execute_decorator_tool_calls(response.content, tools, code_related_tools, state)
    ...
    if len(parts) > 1:
        for i in range(start_index, len(messages)):
            ...
            if isinstance(msg, HumanMessage):
                msg.content = msg.content.replace(test_reminder, "")
    if len(parts) > 2:
