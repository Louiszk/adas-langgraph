...
def main() -> None:
    ...
    parser.add_argument("--system_name", required=True, help="Name of the target system module in 'generated_systems'.")
    parser.add_argument("--state", default="{}", help="JSON string defining the initial state for the workflow.")
    parser.add_argument("--run-id", default=None, help="Unique identifier/timestamp for this run to sync output filenames.")
    ...
    try:
        try:
            ...
            print(f"Warning: Invalid JSON for --state argument. Using an empty state {{}}.")
            initial_state = {}
        ...
        for mode, payload in workflow.stream(initial_state, config={"recursion_limit": 20}, stream_mode=["updates", "values"]):
